...
const BANNED = ["password", "1234", "qwerty"];
...
function validarNombre(value) {
    ...
    if (value !== value.trim())
        return "El apellido no puede tener espacios en blanco al inicio o final.";
}
    //validar correo
    ...
    function requerimientos(pwd, correoVal) {
        ...
        const pwdBanned = [...BANNED, nombre_sitio, nombre, apellido].filter(Boolean);
        return {
            len: pwd.length >= 8, letter: /[a-zA-ZáéíóúÁÉÍÓÚñÑ]/.test(pwd),
            num: /\d/.test(pwd), special: caract_especiales.test(pwd), 
            noBanned: !pwdBanned.some(b => b && c.includes(b)),
    ...
    function validarContrasenia(value) {
        if (!value) return "La contraseña no puede estar en blanco.";
        const correoVal = document.getElementById("correo").value;
        const r = requerimientos(value, correoVal);
    ...
    document.getElementById("correo").addEventListener("input", function () {
        ...
        if (pwd) validarEnTiempoReal(pwd, this.value);
        validarTodo();
    });

    document.getElementById("password").addEventListener("input", function () {
        const correoVal = document.getElementById("correo").value;
        validarEnTiempoReal(this.value, correoVal);
